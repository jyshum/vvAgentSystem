...
def _tables():
    return {
        ...
        # Empty: this fixture models a client with no earlier completed audit,
        # so `_previous_completed_results` finds no baseline and every finding
        # classifies as `new`. The run's own id is generated by the pipeline
        # before the insert, never read back out of this response.
        "technical_audit_runs": _chainable_table([]),
...
def test_run_technical_pipeline_persists_only_evidence_and_community_opportunities(
    ...
    # The returned run id must be the id of the row the pipeline actually
    # created, rather than anything echoed back by the database response.
    audit_run_id = tables["technical_audit_runs"].insert.call_args.args[0]["id"]
    assert result == {
...
def test_run_technical_pipeline_returns_audit_error_without_action_cards(monkeypatch):
    ...
    # The failing run must still be reported under the id of the row that was
    # created, so the caller can trace the run that was marked `error`.
    audit_run_id = tables["technical_audit_runs"].insert.call_args.args[0]["id"]
    assert result == {
        "improvement_run_id": "improvement-run-1",
        "technical_audit_run_id": audit_run_id,
