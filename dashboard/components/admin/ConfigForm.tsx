...
export function ConfigForm({ client }: { client: Client }) {
  ...
  const [sitePlatform, setSitePlatform] = useState(client.site_platform || "unknown");
  const [implementationMode, setImplementationMode] = useState(client.implementation_mode || "copy_paste");
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      ...
      <div className="mb-6 mt-10 pt-8" style={{ borderTop: "1px solid var(--hair)" }}>
        ...
        >
          <option value="unknown" style={{ background: "var(--ink)" }}>Unknown</option>
          ...
          <option value="repository" style={{ background: "var(--ink)" }}>Code Repository</option>
          <option value="other" style={{ background: "var(--ink)" }}>Other</option>
        </select>
        <div className="font-mono text-[8px] mt-1.5" style={{ color: "var(--faint)" }}>
          Tailors the technical audit&apos;s remediation steps to the client&apos;s platform
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <select
          ...
          onChange={(e) => setImplementationMode(e.target.value)}
        >
          <option value="copy_paste" style={{ background: "var(--ink)" }}>Copy & Paste</option>
          <option value="guided" style={{ background: "var(--ink)" }}>Guided</option>
          <option value="github_pr" style={{ background: "var(--ink)" }}>GitHub PR</option>
          <option value="staged_api" style={{ background: "var(--ink)" }}>Staged API</option>
        </select>
        <div className="font-mono text-[8px] mt-1.5" style={{ color: "var(--faint)" }}>
          {implementationMode === "copy_paste" && "Approved changes exported as text — no setup needed"}
          {implementationMode === "guided" && "Step-by-step instructions for applying changes in the platform UI"}
          {implementationMode === "github_pr" && "Auto-creates pull requests with the approved changes"}
          {implementationMode === "staged_api" && "Pushes approved changes to a staging environment via API"}
        </div>
      </div>
